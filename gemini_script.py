...
def insert_website_data(cursor, url, category_data):
    """Inserts website data into the database."""
    try:
        cursor.execute(
        ...
        print(f"Error inserting website data for {url}: {err}")
    return website_id

def insert_social_media_data(cursor, website_id, social_media_data):
    ...
    try:
        ...
        print(f"Error inserting social media data for {url}: {err}")

def insert_tech_stack_data(cursor, website_id, tech_stack_data):
    """Inserts tech stack data into the database."""
    try:
        ...
        print(f"Error inserting tech stack data for {url}: {err}")

def insert_meta_data(cursor, website_id, meta_data):
    """Inserts meta data into the database."""
    try:
        ...
        print(f"Error inserting meta data for {url}: {err}")

def insert_payment_gateways_data(cursor, website_id, payment_gateways_data):
    """Inserts payment gateways data into the database."""
    try:
        ...
        print(f"Error inserting payment gateways data for {url}: {err}")

def insert_language_data(cursor, website_id, language_data):
    """Inserts language data into the database."""
    try:
        ...
        print(f"Error inserting language data for {url}: {err}")
...
if __name__ == "__main__":
    ...
    for url in website_urls:
        ...
        try:
            # Insert Website Data
            website_id = insert_website_data(cursor, url, category_data)

            # Insert Social Media Data
            insert_social_media_data(cursor, website_id, social_media_data)
                 
            # Insert Tech Stack Data
            insert_tech_stack_data(cursor, website_id, tech_stack_data)

            # Insert Meta Data
            insert_meta_data(cursor, website_id, meta_data)

            # Insert Payment Gateways Data
            insert_payment_gateways_data(cursor, website_id, payment_gateways_data)

            # Insert Language Data
            insert_language_data(cursor, website_id, language_data)
            ...
            if e.response.status_code == 401:
                print(f"Forbidden access for {url}. Skipping...")
                continue
            elif e.response.status_code == 403:  # Check for 403
                print(f"Access forbidden for {url}. Skipping...")
                continue   
            elif e.response.status_code == 503: # Check for 503
                print(f"Service Unavailable for {url}. Skipping...")
                continue
            elif e.response.status_code == 429: 
                print(f"Too many requests for {url}. Skipping...")
                continue 
            elif e.response.status_code == 500: 
                print(f"Server error for {url}. Skipping...")
                continue
            elif e.response.status_code == 404: 
                print(f"Resource not found for {url}. Skipping...")
                continue 
            else:
                print(f"Error fetching {url}: {e}")
